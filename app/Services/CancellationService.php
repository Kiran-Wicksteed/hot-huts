...
class CancellationService
{
    ...
    public function calculateRefund(Booking $booking): array
    {
        ...
        if ($hoursUntilSession < 4) {
            return [
                'refund_percentage' => 0,
                'refund_amount_cents' => 0,
                'policy' => 'less_than_4_hours',
                'can_cancel' => true,
                'can_reschedule' => false,
                'message' => 'Cancellation allowed but no refund will be issued (less than 4 hours before session).',
            ];
        }
        ...
        if ($hoursUntilSession < 24) {
            ...
            return [
                ...
                'policy' => '4_to_24_hours',
                'can_cancel' => true,
                'can_reschedule' => true,
                'message' => 'You will receive a 50% refund as a coupon (4-24 hours before session).',
            ];
        }
    ...
    public function cancelBooking(Booking $booking, int $userId, ?string $reason = null): array
    {
        ...
        return DB::transaction(function () use ($booking, $userId, $reason, $refundInfo) {
            ...
            $message = $coupon
                ? "Booking cancelled successfully. Refund coupon code: {$coupon->code} (R{$coupon->remaining_value})"
    ...
    public function getCancellationPreview(Booking $booking): array
    {
        ...
        return array_merge($refundInfo, [
            ...
            'booking_amount_cents' => (int) ($booking->amount * 100),
