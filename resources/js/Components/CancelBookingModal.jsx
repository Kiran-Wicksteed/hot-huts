...
export default function CancelBookingModal({ booking, isOpen, onClose }) {
    ...
    useEffect(() => {
        if (isOpen && booking) {
            ...
            if (typeof route === 'undefined') {
                setPreview({
                    can_cancel: false,
                    message: 'Route helper not available. Please refresh the page.'
                });
                setLoading(false);
                return;
            }
            
            const url = route('bookings.cancel.preview', booking.id);
            ...
            fetch(url)
                ...
                .catch(err => {
                    setPreview({
                        can_cancel: false,
                        message: `Failed to load cancellation details: ${err.message}`
                    });
                    setLoading(false);
                });
        }
    ...
    const modalContent = (
        ...
        >
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" style={{ backgroundColor: 'white' }}>
                {/* Header */}
                            ...
                            <div className="bg-gray-50 rounded-lg p-4 mb-6">
                                ...
                                <div className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium">R{(Number(preview.booking_amount_cents || 0) / 100).toFixed(2)}</span>
                                    </div>
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium">{Math.floor(preview.hours_until_session || 0)} hours</span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div className="bg-white rounded p-3 mt-3">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className="text-lg font-bold text-green-600">
                                                R{(Number(preview.refund_amount_cents) / 100).toFixed(2)}
                                            </span>
                                        </div>
